...
public abstract class ServerList_panel {
    ...
    public static JPanel init() throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, IOException, URISyntaxException {
        if (panel == null) {
            ...
            add_server.setSelectedIcon(get_icon("images/add_server_sel.png"));
            start_dns.setIcon(get_icon("images/power_on.png"));
            start_dns.setPressedIcon(get_icon("images/power_on_pres.png"));
            start_dns.setSelectedIcon(get_icon("images/power_on_sel.png"));
            ...
            stop_dns.setSelectedIcon(get_icon("images/power_off_sel.png"));
    ...
    private static ImageIcon get_icon(String name) throws IOException {
        InputStream iStream = ServerList_panel.class.getClassLoader().getResourceAsStream(name);
        return new ImageIcon(iStream.readAllBytes());
    }

    private static void init_list() throws IOException, InvocationTargetException, InstantiationException, IllegalAccessException, URISyntaxException {
        String file_txt = new String(new FileInputStream(Main_frame.jar_path + "/server_info.dat").readAllBytes());
        ...
        for (String server_info : servers_info) {
            ...
            if (info.length == 5) { //se ci sono i 5 valori
                add_server(new ServerInfo(info[0], info[1], info[2], info[3], info[4])); //aggiunge il server alla lista
            }
        }
    }
